...
from src.config import (
    GROK_MODEL, AZURE_MODEL, EMBED_MODEL_NAME, LLM_PROVIDER,
...
async def lifespan(app: FastAPI):
    ...
    _state.update(index=index, store=store,
...
app = FastAPI(title="LLM Carbon Footprint Research Portal", version="2.0.0", lifespan=lifespan)
...
def health():
    return HealthResponse(
        status="ok", provider=LLM_PROVIDER,
        model=GROK_MODEL if LLM_PROVIDER == "grok" else AZURE_MODEL,
...
def query_rag(req: QueryRequest):
    ...
    try:
        clean = sanitize_query(req.query)
        ...
        with open(MANIFEST_PATH, newline="", encoding="utf-8") as fh:
            for row in csv.DictReader(fh):
                sources.append(CorpusSource(
                    ...
                    source_type=row["source_type"], venue=row["venue"],
                    url_or_doi=row["url_or_doi"],
                    tags=row.get("tags", ""), relevance_note=row.get("relevance_note", ""),
                ))
    strategy = None
    strat_path = PROCESSED_DIR / "chunking_strategy.json"
    if strat_path.exists():
        strategy = json.loads(strat_path.read_text(encoding="utf-8"))
    return {"total_sources": len(sources), "sources": sources, "chunking_strategy": strategy}
...
def get_evaluation():
    ...
    if not files:
        return {"total_queries": len(EVAL_QUERIES), "summary": {}, "results": []}
    results = json.loads(files[-1].read_text(encoding="utf-8"))
    return {"total_queries": len(results), "summary": compute_summary(results), "results": results}
...
def get_logs(limit: int = Query(50, ge=1, le=500)):
    ...
    if not path.exists():
        return {"total_runs": 0, "entries": []}
    raw = [json.loads(line) for line in path.read_text(encoding="utf-8").strip().split("\n") if line.strip()]
    entries = [
        {"run_id": e.get("run_id", ""), "timestamp": e.get("timestamp", ""),
         "mode": e.get("mode", ""), "query": e.get("query", "")[:120],
    ...
    return {"total_runs": len(raw), "entries": entries}
