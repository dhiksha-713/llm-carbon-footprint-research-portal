...
def decompose_query(query: str, client: genai.Client) -> list[str]:
...
def rewrite_query(query: str, client: genai.Client) -> str:
    resp = client.models.generate_content(
        model=GENERATION_MODEL,
        contents=query,
        config=types.GenerateContentConfig(
...
def _build_synthesis_prompt(query: str, chunks: list[dict],
...
def run_enhanced_rag(
    ...
    sub_queries = decompose_query(query, client) if qtype in ("synthesis", "multihop") else [rewritten]
    ...
    resp = client.models.generate_content(
        model=GENERATION_MODEL,
        contents=_build_synthesis_prompt(query, merged, sub_queries, rewritten),
        config=types.GenerateContentConfig(
